...
@section('action-content')
    ...
    <section class="content">
            ...
            <!--Boton para Agregar-->
              <div class="form-group">
                <div class="col-md-7 col-md-offset-5">
                  <a class="btn btn-primary" href="{{ route('transport-operator.create') }}"><i class="glyphicon glyphicon-plus-sign"></i> Nuevo</a>
                </div>
              </div>
        @endcomponent
      ...
      <form method="POST" action="{{ route('transport-operator.search') }}">
         {{ csrf_field() }}
      ...
      <div class="row">
        <div class="col-sm-12">
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead >
              <tr role="row">
                <th width="55%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Operador</th>                
                <th width="30%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Maquinaria</th>
            ...
            @foreach ($dats as $data)
                <tr role="row" class="odd">
                  ...
                  <td class="sorting">{{ $data->machinery }}</td>
                  <td class="sorting hidden-xs">{{ $data->state }}</td>                 
                  <td>
                  <a href="{{ route('transport-operator.edit', ['id' => $data->id]) }}" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                  </td>
                </tr>
              @endforeach
            </tbody>
